...
function getPendingAuthRedirect() {
  try {
    return sessionStorage.getItem(PENDING_AUTH_REDIRECT_KEY) || window.__betsnipePendingAuthRedirect || "";
  } catch (error) {
...
function redirectPendingAuthActionIfNeeded() {
  ...
  if (window.location.href !== pendingUrl && window.location.pathname !== pendingUrl) {
...
function openLoginPopup() {
  const loginButton =
    document.querySelector('button[data-wlc-element="login-button"]') ||
    document.querySelector('[data-wlc-element="login-button"]') ||
    document.querySelector('button[data-wlc-element*="login"]') ||
    document.querySelector('[data-wlc-element*="login"]') ||
    Array.from(document.querySelectorAll("button, a")).find((element) => {
      const text = (element.textContent || "").trim().toLowerCase();
      return text === "entrar" || text === "login" || text === "iniciar sessão";
    });
...
function handleAuthActionClick(event) {
  ...
  if (!opened) {
    window.location.href = "/pt/signup";
  }
}
...
/*
  If the player logs in successfully after the login popup opens, the platform may:
  1) update the <body> class from wlc-body--auth-0 to wlc-body--auth-1 without reloading; or
  2) reload the current page with wlc-body--auth-1.
  These checks cover both cases and then redirect the player to the original clicked link.
*/
