(function () {
  const pageRoot =
    document.getElementById("betsnipe-promo-page-en") ||
    document.getElementById("betsnipe-promo-page-pt");
  if (!pageRoot) return;

  const BETSNIPE_PROMO_VERSION = "en-v20";

  if (window.__betsnipePromoEnLoadedVersion === BETSNIPE_PROMO_VERSION) {
    return;
  }

  window.__betsnipePromoEnLoadedVersion = BETSNIPE_PROMO_VERSION;
  ...
  */
  ...
  /*
    ...
    The four current bonuses are also defined in JavaScript. This avoids
    depending on how WordPress handles <template> and ensures the modal
    receives the terms even if the hidden markup is removed or changed.

    For future promotions, sections present in the DOM are still accepted
    through .modal-sections-template / .modal-section.
  */
  ...
  const BONUS_TERMS_SOURCE =
  {
      "1st Deposit Bonus": [
          ...
          {
              "title": "Use and Wagering Requirements",
      ...
      "1st Casino Deposit": [
          ...
          {
              "title": "Use of Free Spins and Winnings",
          ...
          {
              "title": "Game Rules and Restrictions",
      ...
      "Wednesday Bonus": [
          ...
          {
              "title": "Compatibility with Other Bonuses",
  ...
  const BONUS_TERMS_ALIASES = {
    "First Deposit Bonus": "1st Deposit Bonus",
    "1st Sports Deposit Bonus": "1st Deposit Bonus",
    "First Sports Deposit Bonus": "1st Deposit Bonus",
    "Sports 1st Deposit Bonus": "1st Deposit Bonus",
    "Sports First Deposit Bonus": "1st Deposit Bonus",
    "Bónus de 1º Depósito": "1st Deposit Bonus",

    "Friday Deposit Bonus": "Friday Bonus",
    "Sports Friday Bonus": "Friday Bonus",
    "Bónus de Sexta-feira": "Friday Bonus",

    "First Casino Deposit": "1st Casino Deposit",
    "1st Deposit Casino": "1st Casino Deposit",
    "Casino First Deposit": "1st Casino Deposit",
    "Casino 1st Deposit": "1st Casino Deposit",
    "Casino Welcome Bonus": "1st Casino Deposit",
    "1º Depósito Casino": "1st Casino Deposit",

    "Wednesday Deposit Bonus": "Wednesday Bonus",
    "Casino Wednesday Bonus": "Wednesday Bonus",
    "Bónus de Quarta-feira": "Wednesday Bonus"
  };

  Object.keys(BONUS_TERMS_ALIASES).forEach((aliasTitle) => {
    const sourceTitle = BONUS_TERMS_ALIASES[aliasTitle];
    const sourceKey = normalizeBonusTitle(sourceTitle);
    const aliasKey = normalizeBonusTitle(aliasTitle);

    if (BONUS_TERMS[sourceKey]) {
      BONUS_TERMS[aliasKey] = BONUS_TERMS[sourceKey];
    }
  });
    ...
    if (holder) {
      ...
      if (holderSections.length) {
        return holderSections.map((section, index) => ({
          title: section.dataset.title || `Section ${index + 1}`,
      ...
      accordion.innerHTML = `
        <div class="acc-item active">
          <button
            ...
            aria-expanded="true"
          >
            <span class="acc-title">Promotion terms</span>
            <span
              class="acc-chevron"
              aria-hidden="true"
            ></span>
